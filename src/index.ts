...
import { up as addNewFeatureCreditCosts } from './db/migrations/031_add_new_feature_credit_costs';
import { up as createSavedBatches } from './db/migrations/032_create_saved_batches';
import { up as addFalsePositiveTracking } from './db/migrations/033_add_false_positive_tracking';
import { up as createAbTests } from './db/migrations/034_create_ab_tests';
import { up as createCiCdKeys } from './db/migrations/035_create_ci_cd_keys';
import { up as createEvaluationRubrics } from './db/migrations/036_create_evaluation_rubrics';
import { up as createConversationFlows } from './db/migrations/037_create_conversation_flows';
...
const startServer = async () => {
  try {
    ...
    await addNewFeatureCreditCosts(pool);
    await createSavedBatches(pool);
    await addFalsePositiveTracking(pool);
    await createAbTests(pool);
    await createCiCdKeys(pool);
    await createEvaluationRubrics(pool);
    await createConversationFlows(pool);
